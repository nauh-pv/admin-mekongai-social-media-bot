...
const SidebarUser = ({ local_varaiable, ThemeChanger }: any) => {
  const [menuitems, setMenuitems] = useState(MenuItemsUser);
  ...
  function setMenuUsingUrl(currentPath: any) {
    ...
    setSubmenuRecursively(MenuItemsUser);
  }
    ...
    if (
      ...
      for (const item of MenuItemsUser) {
        if (item === targetObject) {
          ...
          if (item.active) {
            closeOtherMenus(MenuItemsUser, item);
          } else {
            if (theme.dataVerticalStyle == "doublemenu") {
              ThemeChanger({ ...theme, dataToggled: "double-menu-close" });
            }
          }
          setAncestorsActive(MenuItemsUser, item);
        } else if (!item.active) {
          if (theme.dataVerticalStyle != "doublemenu") {
            item.active = false; // Set active to false for items not matching the target
          }
  ...
  function setAncestorsActive(MenuItemsUser: any, targetObject: any) {
    const theme = store.getState().main;
    const parent = findParent(MenuItemsUser, targetObject);
    if (parent) {
      parent.active = true;
      if (parent.active) {
        ThemeChanger({ ...theme, dataToggled: "double-menu-open" });
      ...
      setAncestorsActive(MenuItemsUser, parent);
    } else {
      if (theme.dataVerticalStyle == "doublemenu") {
        ThemeChanger({ ...theme, dataToggled: "double-menu-close" });
      }
  ...
  function closeOtherMenus(MenuItemsUser: any, targetObject: any) {
    for (const item of MenuItemsUser) {
      if (item !== targetObject) {
        item.active = false;
        if (item.children && item.children.length > 0) {
  ...
  function findParent(MenuItemsUser: any, targetObject: any) {
    for (const item of MenuItemsUser) {
          ...
          <SimpleBar>
            <nav className="main-menu-container nav nav-pills flex-column sub-open">
              ...
              <ul className="main-menu" onClick={() => Sideclick()}>
                {MenuItemsUser.map((levelone: any) => (
